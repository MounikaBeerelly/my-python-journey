...
empDataFrameWN = pd.read_csv("C:\Practice\my-python-journey\DataSets\EmpDataSet.csv")
    
print("\nThe data is loaded successfully, redirecting the data to the console...", end="\n")

print(empDataFrameWN)

nullFillRule = input("\nDo you want to drop the null value rows (Y OR NO): ")

if nullFillRule == 'Y' or nullFillRule == 'y' :
   empDataFrameNN = empDataFrameWN.dropna(axis = 0, how = 'any')
else :
    print("\n", empDataFrameWN)

print("\nThe total number of records loaded are: ", len(empDataFrameWN), end="\n")
print("\nThe total number of Null records loaded are: ", len(empDataFrameNN), end="\n")
print("\nThe total number of records found with NULL are: ", (len(empDataFrameWN) - len(empDataFrameNN)), end="\n")
...
Do you want to drop the null value rows (Y OR NO): y

The total number of records loaded are:  14

The total number of Null records loaded are:  4

The total number of records found with NULL are:  10
