...
while loopState :
    ...
    empSerachList.append(searchValue)
    searchChoice = input("\nDo you want to add another employee number for search ('Y' OR 'N') :")
    if searchChoice == 'N' or searchChoice == 'n' :
        print("\nRequest to terminate the search process..", end="\n")
        loopState = False
        
empRecord = empDataFrame.loc[empSerachList]

print("\nThe Details of the employee with Employee number: " + str(empSerachList) + " are..", end="\n")
print(empRecord)
...
Please enter the Employee Number to retrieve the record: 7934

Do you want to add another employee number for search ('Y' OR 'N') :y

Please enter the Employee Number to retrieve the record: 7844

Do you want to add another employee number for search ('Y' OR 'N') :y

Please enter the Employee Number to retrieve the record: 7902

Do you want to add another employee number for search ('Y' OR 'N') :y

Please enter the Employee Number to retrieve the record: 7876

Do you want to add another employee number for search ('Y' OR 'N') :n

Request to terminate the search process..

The Details of the employee with Employee number: [7934, 7844, 7902, 7876] are..
        ENAME       JOB     MGR   HIREDATE   SAL  COMM  DEPTNO
EMPNO
7934   MILLER     CLERK  7782.0  23-Jan-82  1300   NaN      10
7844   TURNER  SALESMAN  7698.0   8-Sep-81  1500   0.0      30
